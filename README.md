...
|---|---|---|
| 1 | 브랜드 브리프를 입력받아 AI로 브랜드 요소를 생성하는 파이프라인을 설명할 수 있다 | `main.py`의 `run_pipeline()` — 6단계 파이프라인으로 구현 |
...
| 4 | API 호출 시 발생할 수 있는 오류 상황과 대응 방법을 설명할 수 있다 | `utils/llm_client.py`의 `call_with_retry()` + 단계별 개별 `try-except`  |
